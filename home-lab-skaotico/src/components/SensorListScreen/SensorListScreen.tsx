...
function HumedadChart({ lecturas }: { lecturas: LecturaSensor[] }) {
  ...
  // En lugar de gráfico, mostrar un resumen de la última lectura y el promedio
  const ultima = lecturas[lecturas.length - 1];
  const promedio = lecturas.reduce((acc, curr) => acc + curr.valor, 0) / lecturas.length;

  return (
    <View style={{ width: '100%', alignItems: 'center', paddingVertical: 16 }}>
      <Text style={{ color: COLORS.teal, fontSize: 32, fontWeight: 'bold' }}>
      ...
      <View style={{ flexDirection: 'row', alignItems: 'center', marginTop: 12 }}>
        <Feather name="trending-up" size={16} color={COLORS.amber} style={{ marginRight: 6 }} />
        <Text style={{ color: COLORS.amber, fontSize: 14 }}>
          Promedio del periodo: {promedio.toFixed(1)}%
        </Text>
      </View>
    </View>
  );
  ...
  return (
    <FadeSlideCard delay={index * 80} style={{ marginRight: 12 }}>
      <PressableScale onPress={onPress}>
        <View style={[styles.sensorCard, isSelected && styles.sensorCardSelected]}>
          {innerContent}
        </View>
      </PressableScale>
    </FadeSlideCard>
