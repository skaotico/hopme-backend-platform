...
El microservicio cuenta con una tubería (*pipeline*) de CI/CD automatizada y robusta definida en el [Jenkinsfile](file:///e:/yosemar/proyectos/home%20lab%20y%20weas%20mias/git/RetroMarket/apis/api-auth/Jenkinsfile). El diseño de esta tubería sigue las mejores prácticas de la industria en **infraestructura como código (IaC)**, **seguridad integrada (DevSecOps)** y **validación nativa en contenedores**.
...
* **Qué hace**: De forma orquestada y limpia, detiene y destruye el contenedor de docker anterior `api-auth` si existiera, y levanta un contenedor fresco en segundo plano (`-d`) mapeando el puerto `9090` y adjuntando el archivo `.env` descargado dinámicamente.
